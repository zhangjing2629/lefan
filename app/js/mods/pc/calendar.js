 ...
 * @date 2016/03
 */
define('mods/pc/calendar', function(require, exports, module) {
    var Calendar = {
        // 元素
        _initDom: function() {
            // 签到日历
        ...
        _changeClass: function(date, className, render) {
            ...
            var calendarTableNodeChild = calendarTableNode.find('li[data-dateid="' + date + '"]');
        ...
        _renderDate: function(days, weekday, classData) {
            ...
            for (j = 0; j < 8; j++) {
                for (i = 0; i < 7; i++) {
                    if (j == 0 && i < weekday) {
                        calendarTableNode.find('ul').eq(j).find('li').eq(i).html('<span></span>');
                        calendarTableNode.find('ul').eq(j).find('li').eq(i).removeAttr('data-dateid').removeClass().removeAttr('data-role');
                    } else {
                        calendarTableNode.find('ul').eq(j).find('li').eq(i).removeClass().removeAttr('data-role');

                        if (a <= days) {
                            // 判断日期是否是今天以后的日期
                            dateid = this.curYear + '/' + this._fixDate(this.curMonth) + '/' + this._fixDate(a);
                            calendarTableNode.find('ul').eq(j).find('li').eq(i).html('<span>' + a + '</span>');
                            calendarTableNode.find('ul').eq(j).find('li').eq(i).attr('data-dateid', dateid);

                            a++;
                        } else {
                            calendarTableNode.find('ul').eq(j).find('li').eq(i).html('<span></span>');
                            calendarTableNode.find('ul').eq(j).find("li").eq(i).removeAttr('data-dateid');
                        }
                    }
                }
            };
        ...
        _bindEvent: function() {
            ...
            // 日历翻月
            // 上一个月
            _this.plSignNode.find('[data-role="btn-calendar-prev"]').on('click', function(e) {
                _this.curMonth--;
                if (_this.curMonth === 0) {
                    _this.curMonth = 12;
                    _this.curYear--;
            ...
            // 下一个月
            _this.plSignNode.find('[data-role="btn-calendar-next"]').on('click', function(e) {
                _this.curMonth++;
